                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        @foreach($cartItems as $item)
                            <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 flex gap-4">
                                
                                <div class="flex-shrink-0 w-32 h-32 rounded-lg overflow-hidden">
                                    <img src="{{ $item['gameImage'] }}"
                                        alt="{{ $item['gameName'] }}" class="w-full h-full object-cover">
                                </div>

                                
                                <div class="flex-1">
                                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">
                                        <a href="{{ route('game.show', $item['gameId']) }}" class="hover:text-miku-500">
                                            {{ $item['gameName'] }}
                                        </a>
                                    </h3>

                                    <div class="flex items-center justify-between">
                                        <p class="text-xl font-bold text-miku-600 dark:text-miku-400">
                                            {{ number_format($item['priceAtTime'], 0, ',', '.') }} đ
                                        </p>

                                        <a href="{{ route('cart.remove', $item['id']) }}" 
                                           class="text-red-500 hover:text-red-700 font-medium transition"
                                           onclick="return confirm('Xóa game này khỏi giỏ hàng?')">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 sticky top-4">
                            ...
                            <div class="space-y-3 mb-6">
                                <div class="flex justify-between text-gray-700 dark:text-gray-300">
                                    <span>Số lượng:</span>
                                    <span class="font-semibold">{{ count($cartItems) }} game</span>
                                </div>
                                <div class="flex justify-between text-gray-700 dark:text-gray-300">
                                    <span>Tạm tính:</span>
                            ...
                            <div class="pt-6 border-t border-gray-200 dark:border-gray-700 mb-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-2xl font-bold text-miku-600 dark:text-miku-400">
                                        {{ number_format($total, 0, ',', '.') }} đ
                                    </span>
                                </div>
                            </div>

                            <button class="block w-full bg-miku-500 hover:bg-miku-600 text-white font-bold py-4 px-6 rounded-xl text-center transition shadow-lg mb-3">
                                <i class="fa-solid fa-lock mr-2"></i>Thanh toán ngay
                            </button>
